...
<?= $this->section('content') ?>
    <div class="wrapper ">
        ...
        <div class="main-panel">
                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12">
                        <div class="card card-stats">
                            <div class="card-body">
                                <h3><i class="fa fa-list-alt" aria-hidden="true"></i> Data Semua Karyawan</h3>
                                <?php if($_SESSION['status'] == 3) { ?>
                                    <a href="<?= base_url(); ?>/dashboard-admin/karyawan/add" class="btn btn-primary">
                                        <i class="fa fa-plus" aria-hidden="true"></i> Tambah Karyawan
                                    </a>
                                <?php } ?>
					            <input type="hidden" id="hidden-csrf" name="<?= csrf_token() ?>" value="<?= csrf_hash() ?>" />
                                <div id="list-staf"></div>
                            </div>
                        </div>
                    </div>
            ...
            <script type="text/babel" id="scripts-list-data" src="<?= base_url() ?>/assets/reactjs/list-karyawan.js?endpoint=<?= base_url(); ?>&status_user=<?= $_SESSION['status']; ?>&nipdata=<?= $_SESSION['nip']; ?>"></script>
